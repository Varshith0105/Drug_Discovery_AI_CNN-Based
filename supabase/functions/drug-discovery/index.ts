...
serve(async (req) => {
  ...
  try {
    ...
    // System prompt that simulates GNN + Transformers + Cross Attention
    const systemPrompt = `You are an advanced drug discovery AI system that uses a hybrid architecture combining:
- Graph Neural Networks (GNN) for molecular structure analysis
- Transformer models for protein sequence understanding
- Cross-attention mechanisms for protein-drug interaction prediction
...
3. Provide binding affinity scores (range: 5.0-9.5, higher is better)
4. Suggest molecular properties (MW, LogP, HBD, HBA)
5. Provide confidence scores for predictions
  ...
  "drugCandidates": [
    {
      ...
      "bindingAffinity": number (5.0-9.5),
...
Be scientifically accurate but creative. Generate realistic SMILES notations and plausible drug candidates.`;
